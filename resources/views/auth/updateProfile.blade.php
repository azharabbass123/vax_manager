<x-layout>
  <section id="hero" style="overflow-y: scroll;">
    <div class="d-flex align-items-center justify-content-center">
      <form method="POST" class="p-4 mt-4 w-50 bg-light rounded" action="/update/{{Auth::user()->id}}">
        @csrf
        @method('patch')
        ...
        <div class="mb-3">
          ...
          <select id="city_id" class="form-select" name="city_id" aria-label="Default select example"></select>
          @error('city_id')
          <div class="alert alert-danger">{{ $message }}</div>
          @enderror
        </div>
